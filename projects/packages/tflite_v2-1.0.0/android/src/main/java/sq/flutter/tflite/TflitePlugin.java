...
public class TflitePlugin implements FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    if (bitmapRaw.getWidth() != inputSize || bitmapRaw.getHeight() != inputSize) {
      ...
      if (inputChannels == 1){
        ...
        canvas.drawBitmap(bitmapRaw, matrix, null);
         ...
         // Save the processed bitmap for debugging/visualization.
    try {
        // Get the cache directory for temporary storage.
        File cacheDir = activity.getCacheDir();
        // Create a file in the cache directory.
        File tempFile = new File(cacheDir, "temp_visualization3.png");
        FileOutputStream fos = new FileOutputStream(tempFile);
        // Compress the bitmap as PNG (you can choose JPEG if you prefer).
        bitmap.compress(Bitmap.CompressFormat.PNG, 100, fos);
        fos.flush();
        fos.close();
        // Log the file path so you can locate it later.
        Log.d("DEBUG", "Visualization saved at: " + tempFile.getAbsolutePath());
    } catch (Exception e) {
        e.printStackTrace();
    }
  ...
  void detectObjectOnImage(HashMap args, Result result) throws IOException {
    ...
    if (model.equals("SSDMobileNet")) {
      ...
      new RunYOLO(args, imgData, BLOCK_SIZE, NUM_BOXES_PER_BLOCK, ANCHORS, THRESHOLD, NUM_RESULTS_PER_CLASS, result, imageHeight, imageWidth).executeTfliteTask();
    }
  }
  ...
  private class RunYOLO extends TfliteTask {
    ...
    long startTime;
    int numClasses;
    final float[][][] output; // New shape: [1][84][numPredictions]
    int numPredictions;
    int imageHeight;
    int imageWidth;
    float scaleX;
    ...
    RunYOLO(HashMap args,
      ...
      this.scaleX = (float) imageWidth / inputSize;
      this.scaleY = (float) imageHeight / inputSize;
      Log.v("size", "**********INPUT "+inputSize);
      this.numClasses = labels.size(); // Should be 80 for YOLOv8n
      numPredictions = 8400; // as provided by the model output
      this.output = new float[1][84][numPredictions];
    }
...
protected void onRunTfliteDone() {
  try {
    ...
    Log.v("TflitePlugin", "Inference took " + inferenceTime + " ms");

    // Bail out early if output or labels are missing
    if (output == null || output.length == 0 || output[0].length < 4 + numClasses) {
      Log.w("TflitePlugin", "Invalid TFLite output buffer or insufficient shape");
      result.success(new ArrayList<Map<String, Object>>());
      return;
    }
    if (labels == null || labels.isEmpty()) {
      Log.w("TflitePlugin", "No labels available for YOLO detection");
      result.success(new ArrayList<Map<String, Object>>());
      return;
    }

    // Priority queue sorted by confidence descending
    PriorityQueue<Map<String, Object>> pq = new PriorityQueue<>(
      (lhs, rhs) -> Float.compare(
        (float) rhs.get("confidenceInClass"),
        (float) lhs.get("confidenceInClass")
      )
    );

    // Run through all predictions
    for (int i = 0; i < numPredictions; i++) {
      // Guard against malformed slices
      if (output[0][0].length <= i || output[0][3].length <= i) {
        continue;
      }

      float xCenter = output[0][0][i];
      float yCenter = output[0][1][i];
      float boxW    = output[0][2][i];
      float boxH    = output[0][3][i];

      // Pull class scores
      float[] classes = new float[numClasses];
      for (int c = 0; c < numClasses; c++) {
        int scoreIndex = 4 + c;
        if (output[0][scoreIndex].length <= i) {
          classes[c] = 0f;
        } else {
          classes[c] = output[0][scoreIndex][i];
        }
      }
      ...
      if (bestClass >= 0) {
        ...
        float xmin = xCenter - boxW  / 2f;
        float ymin = yCenter - boxH  / 2f;
        ...
        detection.put("rect",               rect);
        detection.put("confidenceInClass",  bestScore);
        detection.put("detectedClass",      labels.get(bestClass));

        pq.add(detection);
      }
    }

    // Take up to numResultsPerClass per class
    Map<String, Integer> counts = new HashMap<>();
    List<Map<String, Object>> finalResults = new ArrayList<>();
    while (!pq.isEmpty()) {
      Map<String, Object> det = pq.poll();
      String cls = det.get("detectedClass").toString();
      int    cnt = counts.getOrDefault(cls, 0);
      if (cnt < numResultsPerClass) {
        counts.put(cls, cnt + 1);
        finalResults.add(det);
      }
    }

    Log.v("TflitePlugin", "Returning " + finalResults.size() + " detections");
    result.success(finalResults);

  } catch (NullPointerException npe) {
    Log.e("TflitePlugin", "NullPointerException in onRunTfliteDone", npe);
    result.success(new ArrayList<Map<String, Object>>());
  } catch (Exception ex) {
    Log.e("TflitePlugin", "Unexpected exception in onRunTfliteDone", ex);
    result.error("INFERENCE_ERROR", ex.getMessage(), null);
  }
}
